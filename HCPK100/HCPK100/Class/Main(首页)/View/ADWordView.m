...
- (id)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        self.layer.cornerRadius = 13 * Scale;
    }
    return self;
}
...
-(void) _initUI{
    ...
    UIImageView *labaImageView = [[UIImageView alloc] initWithFrame:CGRectMake(15 * Scale, 5.5 * Scale, 15 * Scale, 15 * Scale)];
    labaImageView.image = [UIImage imageNamed:@"home_board_icon"];
    [self addSubview:labaImageView];
    
    gonggaoBGView = [[UIView alloc] initWithFrame:CGRectMake(40* (ScreenW/375), 0, ScreenW - 60 * (ScreenW/375), 26* (ScreenW/375))];
...
-(void) startAnimationWithString:(NSString*)text{
    ...
    gonggaoLabel.font = [UIFont systemFontOfSize:13];
    CGSize msgSize = [gonggaoLabel.text sizeWithAttributes:[NSDictionary dictionaryWithObjectsAndKeys:[UIFont systemFontOfSize:13],NSFontAttributeName, nil]];
    gonggaoLabel.frame = CGRectMake(0, 0, msgSize.width, 33 * (ScreenW/375));
    ...
    moveAnim.duration = gonggaoBGView.frame.size.width * 3 * 0.015;
